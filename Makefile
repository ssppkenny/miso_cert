...
repl: update
	wasm32-wasi-cabal repl cert-exe -finteractive --repl-options='-fghci-browser -fghci-browser-port=8080'

watch:
	ghciwatch --after-startup-ghci :main --after-reload-ghci :main --watch app/*.hs --debounce 50ms --command 'wasm32-wasi-cabal repl cert-exe -finteractive --repl-options="-fghci-browser -fghci-browser-port=8080"'

build:
	wasm32-wasi-cabal build
	rm -rf public
	cp -r static public
	for dir in resources/chapter*/; do \
		find "$$dir" -maxdepth 1 -name '*.png' | sed 's|.*/||' | sort > "$$dir"images.txt; \
	done
